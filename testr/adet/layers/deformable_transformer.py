...
from testr.adet.utils.misc import inverse_sigmoid
from testr.adet.layers.ms_deform_attn import MSDeformAttn
...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed, text_embed, text_pos_embed, text_mask=None):
        ...
        return hs, hs_text, init_reference_out, inter_references_out, enc_outputs_class, enc_outputs_coord_unact, reference_points
...
class DeformableCompositeTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, tgt_text, query_pos_text, reference_points, src, src_spatial_shapes, level_start_index, src_padding_mask=None, text_padding_mask=None):
        ...
        # breakpoint()
        # tmp=0
        # tmp+=sum(i.numel() for i in self.attn_cross.parameters())
        # tmp+=sum(i.numel() for i in self.norm_cross.parameters())

        # tmp+=sum(i.numel() for i in self.attn_intra.parameters())
        # tmp+=sum(i.numel() for i in self.norm_intra.parameters())

        # tmp+=sum(i.numel() for i in self.attn_inter.parameters())
        # tmp+=sum(i.numel() for i in self.norm_inter.parameters())

        # tmp+=sum(i.numel() for i in self.linear1.parameters())
        # tmp+=sum(i.numel() for i in self.linear2.parameters())
