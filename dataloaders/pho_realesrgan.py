...
from .pho_utils import load_file_list, center_crop_arr, random_crop_arr, augment
...
def collate_fn_real(batch):
    ...
    return gt, list(kernel), list(kernel2), list(sinc_kernel), list(prompt), list(text), list(bbox), list(poly_tensor), list(text_enc_tensor), list(img_name)
